...
def _infer_core_tokens(special_tokens: list[str]) -> tuple[str, str, str, str]:
    """从special_tokens 推断 scheme 名称与 pad/bos/eos/unk 的字符串形式
...
def _write_tokenizer_config(
    *,
    ...
    (out_dir / "tokenizer_config.json").write_text(json.dumps(cfg, ensure_ascii=False, indent=4), encoding="utf-8")
...
def train_text_bpe(
    *,
    ...
    # 保证特殊token的ID稳定（SeekerOmniLM的词嵌入拆分依赖这一点）
    for i, t in enumerate(special_tokens):
