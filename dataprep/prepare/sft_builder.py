...
def _detect_resume_start(out_dir: Path, *, num_samples: int, seq_len: int, rewind: int) -> int:
    ...
    att_path = out_dir / 'attention.mask.bin'
...
def build_sft_text_memmap_dataset(
    *,
    ...
    use_chatml = sys_id is None and usr_id is None and asst_id is None and sep_id is None
    ...
    pbar = tqdm(total=int(n), initial=int(start_idx), desc=f'sft-memmap:{out.name}', unit='samples')
    ...
    try:
        for idx, obj in enumerate(_iter_json_objects(jsonl_path)):
            ...
            if conversations is not None:
                ...
                attention_mask = np.zeros((int(max_seq_len),), dtype=np.int8)
                ...
                if not bool(use_chatml):
                    ...
                    # 为了适配max_seq_len,尽量截断prompt/system，同时保留answer
