...
def load_yaml(path: str | Path) -> dict[str, Any]:
    ...
    def _load(cur: Path) -> dict[str, Any]:
        ...
        def _include(loader: yaml.Loader, node: yaml.ScalarNode):
...
class ModelConfig:
    ...
    # 特殊token如何占据词表前部的ID
    # 当前默认只使用minimind2_chatml:<|endoftext|>/<|im_start|>/<|im_end|> + <img_*>
    ...
    # FFN(前馈层)
    mip_intermediate_size: int | None = None
...
class DataConfig:
    ...
    # 可选的多数据集混合（按batch采样）开启后，train_dir视为默认数据集
    # 训练会按mix_weights的概率从mix_train_dirs中抽取batch
...
class TrainConfig:
    ...
    # 冻结相关控制（减少遗忘 / 适配小显存）
    # 当freeze_backbone=True时，可选只解冻最后N个block
    ...
    # 训练可视化(TensorBoard)
    tb_enable: bool = True
    tb_dir: Path | None = None # 默认：outputs/tb/<experiment>/<stage>(与checkpoints下out_dir的相对结构一致)
...
class ExperimentConfig:
    ...
    def load(path: str | Path) -> 'ExperimentConfig':
        ...
        if base is not None:
            ...
            m = m_raw
        mip_intermediate_size = m.get('mlp_intermediate_size')
        ...
        model = ModelConfig(
            ...
            mip_intermediate_size=(int(mip_intermediate_size) if mip_intermediate_size is not None else None),
