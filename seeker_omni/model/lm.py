...
class SeekerOmniLM(nn.Module):
        ...
        self.blocks = nn.ModuleList(
            [
                SeekerBlock(
                    ...
                    cfg.num_kv_heads,
                    dropout=cfg.dropout,
                    intermediate_size=cfg.mip_intermediate_size,
                )
        ...
        *,
        ...
        if labels is not None:
            ...
            if image_feats is not None:
                # 加权损失：强调监督区间的前若干token（有助于描述任务在贪心解码下正确起步）
                ...
                if bool(has.any()):
                    first = torch.argmax(token_loss, dim=1)
                    k = 16
                    alpha = 3.0
    ...
    def _protected_intervals(self, input_ids: torch.Tensor) -> list[list[tuple[int, int]]]:
        """返回每个样本需要保护的[start,end)区间，避免streaming prefill时把区间切开"""
        ...
        def generate_text(
            ...
            *,
            ...
            - 'prefill_chunk_size' 启用streaming prefill(分块处理prompt)，同时避免在模态占位符区间内切分
            
            """
            self.eval()

            if input_ids.ndim != 2:
                raise ValueError(f"input_ids must be [B, 5], got {tuple(intput_ids.shape)}")
            ...
            # attention_mask 作为key mask(1=有效 key), 本代码库只做key侧mask
            ...
            def apply_repetition_penalty(logits: torch.Tensor, ids: torch.Tensor) -> None:
                ...
                for b in range(int(ids.shape[0])):
                    ...
                    vals = logits[b, uniq]
                    pos = vals > 0
                    vals = torch.where(pos, val / rep, vals * rep)
                    logits[b, uniq] = vals
            ...
            def sample_from_logits(logits: torch.Tensor) -> torch.Tensor:
                ...
                if 0.0 < top_p < 1.0:
                    ...
                    cut = cum > float(top_p)
                    cut[:, 0] = False
                    sorted_logits = torch.where(cut, torch.full_like(sorted_logits), sorted_logits)
