...
def _prepare_batch(batch: dict, *, device: torch.device):
    ...
    if attention_mask_cpu is None:
        ...
        # 如果attention_mask全为1，表示完全packed、无padding；此时不传给SDPA,
        # 以便使用flash attention 并减少显存占用
...
def _corrupt_answer_tokens_for_mm(
    ...
    *,
    ...
    """在*input*中扰乱监督答案token，以降低教师强制。

    这样前几个助手token会更多依赖提示词 + 模态特征(I/A),
    而不是上下文里已经包含了金标答案前缀
    """
    mask = labels != -100
...
def _adamw_param_groups(model: SeekerOmniLM, *, weight_decay: float) -> list[dict]:
    """为AdamW 构建参数组，并对权重衰减做选择性过滤
    
    说明：
    - 不对norm、词嵌入、模态适配器、门控 做权重衰减
    - 门控参数初始接近0，强L2正则下可能比较脆弱
    """
...
def train(cfg: ExperimentConfig) -> None:
    ...
    if mix_enabled:
        ...
        if int(ds.meta.vocab_size) != int(cfg.model.vocab_size):
            raise ValueError(f'vocab_size mismatch: data={ds.meta.vocab_size} model={cfg.vocab_size}')
    ...
    if mix_enabled:
        pbar = tqdm(total=int(total_steps), initial=int(step), desc='train(mix)', unit="step", dynamic_ncols=True)
        while step < int(total_steps):
            ...
            if tb_writer is not None and tb_every > 0 and (step % tb_every == 0):
                with torch.no_grad():
                    tb_writer.add_scalar("train/loss", float(out.loss.detach().to(torch.float32).cpu()), global_step=int(step))
                    tb_writer.add_scalar("train/ppl", float(torch.exp(out.loss.detach().to(torch.float32)).cpu()), global_step=int(step))
                    ...
                    tb_writer.add_scalar(
                        ...
                        float(torch.tanh(model.img_gate).abs().mean().detach().to(torch.float32).cpu()),
    ...
    if cfg.train.max_steps is None:
        # 基于step：进度显示为step/max_steps(避免提前停止时epoch进度产生误解)
        dl_iter = iter(dl)
        pbar = tqdm(total=int(total_steps), initial=int(step), desc='train', unit="step", dynamic_ncols=True)
        ...
        while step < int(total_steps):
            ...
            step += 1
            lr = consine_lr(step, base_lr=cfg.train.lr, total_steps=int(total_steps), warmup_steps=cfg.train.warmup_steps)
            for g in opt.param_groups:
                g['lr'] = lr

            intput_ids, labels, attention_mask, image_feats = _prepare_batch(batch, device=device)
    ...
    for epoch in range(cfg.train.epochs):
        pbar = tqdm(total=len(dl), desc=f'train epoch {epoch}', unit="batch", dynamic_ncols=True)
        for batch in dl:
            step += 1
            lr = cosine_lr(step, base_lr=cfg.train.lr, total_steps=int(total_steps), warmup_steps=cfg.train.warmup_steps)
            for g in opt.param_groups:
